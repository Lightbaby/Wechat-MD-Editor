...
5.  **暗夜霓虹 (Cyberpunk)**：虽然微信不支持暗黑模式，但可以使用深色块+高亮文字风格。*(注：考虑到微信阅读体验，建议改为“杂志风 (Magazine)”，大色块拼接)*
...
## 📋 实施计划
- [ ] **Step 1: 主题扩展 (constants.ts)** (1h)
    - 定义 5 个新主题的样式对象。
- [ ] **Step 2: UI 框架重构 (App.tsx)** (1.5h)
    - 引入 Sidebar 组件。
    - 实现 Drawer/Panel 机制。
    - 调整 Flex 布局。
- [ ] **Step 3: 主题面板开发 (components/ThemePanel.tsx)** (1h)
    - 实现网格布局。
    - 设计主题预览卡片 (Color Palette Preview)。
- [ ] **Step 4: 细节打磨 & 响应式适配** (0.5h)
    - 确保移动端适配（或提示仅桌面端可用）。

## ✅ 验收标准
- [ ] 界面成功切换为侧边栏布局。
- [ ] 主题选择器支持网格展示，且能显示 8+ 个主题。
- [ ] 新增的 5 个主题渲染正常，无样式冲突。
- [ ] AI 工具等原有功能在布局调整后依然可用。

## 📦 边界
- 不包含：用户自定义主题（保存到本地存储暂不实现，仅硬编码）。
- 不包含：暗黑模式编辑器（指编辑器本身的 Dark Mode，非渲染内容）。
